...
Mobify.UI.Carousel = (function($, Utils) {
    ...
    Carousel.prototype.bind = function() {
        ...
        function drag(e) {
            ...
            //初始化运动轴
            if(axis === 'X'){
                if(dragThresholdMet || abs(dx) > dragRadius){
                    ...
                    canceled = true;
                    console.log('canceled x')
                }
            }else if(axis === 'Y'){
                if(dragThresholdMet || abs(dy) > dragRadius){
                    dragThresholdMet = true;
        ...
        function end(e) {
            ...
            if (!canceled && axis === "X" && abs(dx) > opts.moveRadius) {
            ...
            } else if( !canceled && axis === "Y" && abs(dy) > opts.moveRadius){
                // Move to the next slide if necessary
                if (dy > 0) {
                    self.next();
                } else {
