...
define(["OE","Menu","lazyload","_","Loading","Swipe","AsideWindow"],function(OE,Menu,lazyload,_,Loading,Swipe,AsideWindow){
    console.log('Page Module: destination');
    var $ = OE.$;
    var u = OE.utils;
    var times = 1;
    function fn(){
        OE.log('Page:[destination], Times:' ,times++);
        Loading.hide();
        $.get(OE.BASE_URL+'/template/trip/tpl_destination.html', function(html){
            console.log('template [tpl_destination.html] onload');
            var $html = $(html);
