...
define(["OE","Menu","lazyload","_","Loading","Swipe","AsideWindow"],function(OE,Menu,lazyload,_,Loading,Swipe,AsideWindow){
    console.log('Page Module: destination list');
    var $ = OE.$;
    var u = OE.utils;
    var times = 1;
    function fn(){
        OE.log('Page:[destination list], Times:' ,times++);
        Loading.hide();
        $.get(OE.BASE_URL+'/template/trip/tpl_destination_list.html', function(html){
            console.log('template [tpl_destination_list.html] onload');
            var $html = $(html);
